...
class phpjs {
    ...
    var $context = array('global_scope');
    var $array_brace_count = 0;
    var $array_assoc_count = array();
...
//        $this->_print_tokens( $tokens ); //exit;
        $this->boilerplate();
...
//echo $var_name . ' : ' . $is_decl . "\n";        
        ...
        if( $this->in_global_decl() ) {
            // variables are global by default in JS.  no need to declare them.  but we do want it in local scope so we don't try to redeclare later.
            $this->local_scope[count($this->local_scope)-1][$var_name] = 1;
            return;
        }
...
//            echo "\n";
    ...
    function in_global_decl() {
        return $this->context(0) == 'global';
    }
